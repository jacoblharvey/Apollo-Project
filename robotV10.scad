...
module frame(){ // 80x100
  ...
  translate([0,36,25])cube([85,55,10]);   // breadboard
    ...
    translate([0,30,0])cube([85,5,5]);
    translate([0,25,0])cube([85,5,5]);
    
    translate([0,125,0])cube([5,5,20]);
...
module spike (){
    ...
    difference(){
    translate([-5,-5,0])cube([10,85,5]);
    translate([-30,13,-4])rotate([0,0,-20])cube([10,85,15]);
    translate([0,0,-1])servoArm();
    }   
